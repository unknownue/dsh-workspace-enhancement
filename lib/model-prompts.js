...
export const MODEL_PROMPTS = {
    ...
    sectionBash: 'The bash tool runs `bash -c` on this session\'s remote Linux workspace. Check the [exit code: N] marker of each result.',
    /**
     ...
     */
    remoteNoSandbox: 'Remote commands and file tools follow this session\'s /permission sandbox (workspace-write, read-only, or danger-full-access), enforced by the Linux core when it is deployed. Confined writes and bash FAIL CLOSED if that core is missing. Reads then fall back to unfenced SFTP so chat and official Read still work (visible: the write/bash refusal names the missing core or bwrap). danger-full-access (including a one-shot sandbox_permissions grant) uses `dsh-core serve --sandbox off`, or today\'s SFTP and SSH when no core is installed. Interactive terminals stay refused while the session is confined. Escalation uses the same official card as local writes.',
    /**
     ...
     */
    remoteGateActive: 'Commands on this machine additionally require an approval decision before they run; do not retry a rejected command unchanged. If the approval gate and sandbox escalation are both on, the operator sees two cards.',
    /**
