...
function clientEnvelopeOf(body) {
...
function envelopeError(rpcId, message) {
 ...
 */
export function channelRouteOf(endpoint, resolve) {
    ...
    return {
        ...
        async fetch(request) {
            ...
            if (envelope === undefined) {
                return envelopeError(body?.rpcId, 'invalid client-request message');
            }
            if (envelope.method !== wireEndpoint) {
                return envelopeError(envelope.rpcId, `method ${JSON.stringify(envelope.method)} does not match endpoint ${JSON.stringify(wireEndpoint)}`);
            }
