 ...
 */
export async function awaitOutcomeOrStop(done, signal, windowMs = STOP_SETTLE_WINDOW_MS) {
    if (signal === undefined)
        return done;
    return new Promise((resolve, reject) => {
        ...
        void done.then((outcome) => { cleanup(); resolve(outcome); }, (error) => { cleanup(); reject(error); });
    });
}
/* ------------------------------------------------------------------- spawn */
...
export function registerWin32Bash(ctx, registry, options = {}) {
    ...
    const inject = (target) => {
        try {
            ...
            if (typeof target.effect === 'function')
                target.effect(() => disposer, 'win32 bash tool');
        }
        catch {
    ...
    if (options.forceRegister === true) {
        ...
        const onCreated = (payload) => {
            ...
            if (cwd === undefined || worldOfCwd(cwd) !== 'remote')
                return;
        ...
        ctx.effect(() => ctx.on('agent/created', onCreated), 'win32 bash session inject');
    }
    const sectionDisposer = ctx.systemPrompt.section({
        name: 'tool:bash',
        order: 105,
