...
import { listRemoteLevel, listRemoteLevelViaCore, mkdirRemoteViaCore, remoteHome as sharedRemoteHome } from "./listing.js";
...
import { deployCore } from "./core-deploy.js";
...
export function apply(ctx, config) {
    ...
    const operatorCore = async (id, path, signal) => {
        try {
        ...
        try {
            switch (endpoint) {
                ...
                case 'core.deploy': {
                    ...
                    const id = input.id.trim();
                    const connection = requireConnection(id);
                    const view = await deployCore(connection, { signal });
                    hub().close(id);
