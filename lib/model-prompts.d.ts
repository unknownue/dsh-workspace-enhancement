...
export declare const MODEL_PROMPTS: {
    ...
    readonly sideItem: "- Side workspace **{label}**: `{rootKey}`";
    ...
    readonly sideNote: "A side workspace is an extra directory this session can operate on directly. Commands run in the main workspace by default; to run a command on another server use `sw_exec(server, command)`.";
    /**
     ...
     */
    readonly connectedHeading: "**Machines connected to this session (`sw_exec` targets)**:";
    /** REQ-I11: one connected-machine line — id, endpoint, honest reachability. */
    readonly connectedItem: "- `{id}` — {endpoint}{note}";
    /** REQ-I11: the note appended to a machine that did not answer its ping. */
    readonly connectedUnreachable: " (was unreachable at connect time)";
     ...
     */
    readonly sectionSwExec: "sw_exec executes a command on the specified server. The server must be connected to this session: use an id from the connected-machine list, or call sw_connect first. workdir defaults to that server's primary workspace. Check the [exit code: N] marker of each result; investigate non-zero exits before continuing.";
    /** `tool:bash` section (order 105, win32 hosts) — empty unless this session has a remote workspace. */
    readonly sectionBash: "The bash tool runs `bash -c` on this session's remote Linux workspace. Check the [exit code: N] marker of each result.";
    /**
     ...
     */
    readonly remoteNoSandbox: "Remote commands and file tools follow this session's /permission sandbox (workspace-write, read-only, or danger-full-access), enforced by the Linux core when it is deployed. Confined writes and bash FAIL CLOSED if that core is missing. Reads then fall back to unfenced SFTP so chat and official Read still work (visible: the write/bash refusal names the missing core or bwrap). danger-full-access (including a one-shot sandbox_permissions grant) uses `dsh-core serve --sandbox off`, or today's SFTP and SSH when no core is installed. Interactive terminals stay refused while the session is confined. Escalation uses the same official card as local writes.";
    /**
     ...
     */
    readonly remoteGateActive: "Commands on this machine additionally require an approval decision before they run; do not retry a rejected command unchanged. If the approval gate and sandbox escalation are both on, the operator sees two cards.";
    /**
