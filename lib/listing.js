 ...
 */
export async function listRemoteLevelViaCore(client, target, limit, opts) {
    const signal = opts?.signal;
    signal?.throwIfAborted();
    ...
    for (const item of listed) {
        ...
        if (type !== 'directory' && type !== 'symlink')
            continue;
        if (boundedInsert(window, { name: row.name, type }, keep))
            evicted = true;
    }
    const entries = [];
    let truncated = evicted;
...
export async function mkdirRemoteViaCore(client, parent, name, signal) {
