...
export function worldOfCwd(cwd, platform = process.platform) {
    ...
    if (cwd.startsWith('ssh://') && cwd.slice('ssh://'.length).startsWith('.'))
        return 'remote';
...
export class MixedFileSystem {
    ...
    async readByteRange(target, range, signal) {
        if (worldOfTargetKey(String(target.targetKey)) === 'remote') {
            ...
            if (reader === undefined) {
                throw new FsError(`cannot read "${target.displayPath}": the remote filesystem backend does not support windowed reads`, 'FS_IO_ERROR');
            }
