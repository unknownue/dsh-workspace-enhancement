...
import { CoreSubprocessHandle } from "./core-process.js";
import { isConfinedSandboxMode, resolveRemoteSessionMode } from "./remote-policy.js";
/**
...
export class SshSubprocessEngine {
    ...
    lazyTerminalGuard;
    disposing = false;
    constructor(ctx, gate, fence, terminalGuard, hub) {
        this.ctx = ctx;
        this.gate = gate;
        this.fence = fence;
     ...
     */
    sandboxFence() {
        this.lazyFence ??= this.fence ?? createRemoteSandboxFence(this.ctx, this.hub !== undefined ? { hub: this.hub } : {});
    ...
    spawn(spec) {
        ...
        if (this.hub !== undefined && route.connectionId !== undefined) {
            ...
            const sshFallback = isConfinedSandboxMode(policy)
                ? undefined
                : () => new SshSubprocessHandle(route.transport, route.cwd, spec, this.spillDir, undefined, async (argv) => argv);
