 ...
 */
export function vendorCachePath(source) {
 ...
 */
export function cachedVendor(source) {
    const binary = vendorCachePath(source);
    const digest = `${binary}.sha256`;
    if (!existsSync(binary) || !existsSync(digest))
        return undefined;
    try {
 ...
 */
function curlDownload(url, target, proxy) {
    ...
    try {
        ...
        if (run.error !== undefined)
            return `curl could not run (${run.error.message})`;
        if (run.status !== 0)
            return detail === '' ? `curl exited ${String(run.status)}` : detail;
        return undefined;
    }
    finally {
        closeSync(fd);
...
function unavailableDetail(source, reason) {
    return [
 ...
 */
export function ensureVendor(source, download = curlDownload) {
    const hit = cachedVendor(source);
    if (hit !== undefined)
        return { ok: true, path: hit, cached: true };
    if (offline())
        return { ok: false, detail: unavailableDetail(source, 'DSW_CORE_VENDOR_OFFLINE is set') };
    ...
    try {
        ...
        if (failure !== undefined)
            return { ok: false, detail: unavailableDetail(source, failure) };
        if (!existsSync(archive))
            return { ok: false, detail: unavailableDetail(source, 'the download produced no file') };
        ...
        if (extracted !== undefined)
            return { ok: false, detail: unavailableDetail(source, extracted) };
