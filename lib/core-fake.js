 ...
 */
import { mkdirSync, readFileSync, rmSync, statSync, writeFileSync, readdirSync, lstatSync, existsSync } from 'node:fs';
import { dirname, join, posix, resolve, sep } from 'node:path';
import { CORE_ARTIFACT_VERSION, CORE_CAPS, CORE_ERROR_EXISTS, CORE_ERROR_IO, CORE_ERROR_NOT_DIR, CORE_ERROR_PERMISSION, CORE_ERROR_READ_ONLY, CORE_ERROR_UNIMPLEMENTED, CORE_EVENTS, CORE_METHODS, CORE_PROTO, asRecord, decodeFrames, encodeFrame, methodAllowed, } from "./core-protocol.js";
import { fileContentVersion } from "./fs-version.js";
function hostPath(root, posixPath) {
...
function posixOfHost(root, host, fallback) {
...
function realpathAllowMissingHost(root, posixPath) {
    const tryExisting = (path) => {
    ...
    while (true) {
        ...
        if (realAnc !== undefined)
            return missing.length === 0 ? realAnc : posix.join(realAnc, ...missing);
...
function writable(options, posixPath) {
    if (options.sandbox === 'off' || options.sandbox === undefined)
        return true;
 ...
 */
export function serveFakeCore(stdin, stdout, options) {
...
export function resetFakeRoot(root) {
    rmSync(root, { recursive: true, force: true });
    mkdirSync(root, { recursive: true });
}
//# sourceMappingURL=core-fake.js.map