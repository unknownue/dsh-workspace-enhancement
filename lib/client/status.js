...
export function ConnStatusBadge({ id, rpc, center, compact = false, t: tSeat, }) {
    ...
    return (_jsxs("span", { className: styles.badge, children: [_jsx("span", { className: dotClass, title: view?.message !== undefined && view.message !== '' ? view.message : t(CONN_STATE_LABEL_KEY[state]) }), _jsx("span", { className: busy ? `${styles.label} ${styles.labelBusy}` : styles.label, children: label }), state !== 'active' && (_jsx("button", { type: "button", title: actionTitle, disabled: busy, className: styles.retry, onClick: () => { void reconnect(); }, children: compact ? t('status.retryAction.compact') : t('status.retryAction.full') }))] }));
}
